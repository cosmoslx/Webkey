...
static char *jsonEscape(const char *buf, int len) {
  ...
  char *result;
  check(result                = (char*)malloc(count + 1));
  char *dst                   = result;
  ptr                         = buf;
  for (int i = 0; i < len; i++) {
    unsigned char ch          = *(unsigned char *)ptr++;
...
static int completePendingRequest(struct Session *session,
  ...
  if (!session->http) {
    if (len) {
      if (session->buffered) {
        check(session->buffered = (char*)realloc(session->buffered,
                                          session->len + len));
        memcpy(session->buffered + session->len, buf, len);
        session->len           += len;
      } else {
        check(session->buffered = (char*)malloc(len));
    ...
    if (session->buffered) {
      ...
      if (maxLength > 0 && len > maxLength) {
        ...
        check(session->buffered = (char*)malloc(session->len));
...
//    HttpConnection *http        = session->http;
    char *response              = stringPrintf(NULL,
                                             ...
                                             "%s",
                                             (long)strlen(json),
                                             "json");//strcmp(httpGetMethod(http),
                                                 //   "HEAD") ? json : "");
    free(json);
    session->http               = NULL;
