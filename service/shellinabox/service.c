...
void initService(struct Service *service, const char *arg) {
  ...
  service->id                               = -1;
  check(service->path                       = (const char*)malloc(ptr - arg + 2));
  ((char *)service->path)[0]                = '/';
...
//                                                           &service->user);
      ...
      *ptr                                  = '\000';
      service->gid =0;//                         = parseGroupArg(arg,
                        //                                    &service->group);
    }
    *ptr                                    = ':';
    arg                                     = ptr + 1;
...
struct Service *newService(const char *arg) {
  ...
  check(service = (Service*)malloc(sizeof(struct Service)));
...
static int enumerateServicesHelper(void *arg ATTR_UNUSED,
  ...
  check(services              = (Service**)realloc(services,
