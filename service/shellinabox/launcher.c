...
static int read_string(int echo, const char *prompt, char **retstr) {
  *retstr                = NULL;
  ...
  for (;;) {
    ...
    check(line           = (char*)calloc(1, lineLength));
...
int launchChild(int service, struct Session *session, const char *url) {
  ...
  for (int i; u[i = strcspn(u, "\\\"'`${};() \r\n\t\v\f")]; ) {
    ...
    check(u            = (char*)realloc(u, strlen(u) + 4));
  ...
  check(request        = (LaunchRequest*)calloc(len, 1));
...
//  if (!pw->pw_gecos ) pw->pw_gecos  = (char *)"";
  ...
  check(passwd                      = (struct passwd*) calloc(sizeof(struct passwd) +
                                             strlen(pw->pw_name) +
                                             strlen(pw->pw_passwd) +
//                                             strlen(pw->pw_gecos) +
                                             strlen(pw->pw_dir) +
...
static void execService(int width ATTR_UNUSED, int height ATTR_UNUSED,
  ...
  int argc                    = 0;
  char **argv;
  check(argv                  = (char**)malloc(sizeof(char *)));
  key                         = NULL;
  value                       = NULL;
      ...
      if ((quote == NONE || quote == DOUBLE) && ptr[1] == '{') {
        ...
        *end                  = ch;
        ...
        if (incr > 0) {
          ...
          check(cmdline       = (char*)realloc(cmdline,
        ...
        if (state == ENV && value) {
          ...
          for (char **e = environment; *e; e++, numEnvVars++) {
            if (!strncmp(*e, key, len) && (*e)[len] == '=') {
              ...
              check(*e        = (char*)realloc(*e, s_size + 1));
              (*e)[len + 1]   = '\000';
          ...
          if (numEnvVars >= 0) {
            check(environment = (char**)realloc(environment,
          ...
          check(argv          = (char**)realloc(argv, (argc + 1)*sizeof(char *)));
        }
      }
      key                     = NULL;
      value                   = NULL;
  ...
  if (service->useDefaultShell) {
    ...
    check(argv[0]             = (char*)realloc(argv[0], len + 2));
...
static void childProcess(struct Service *service, int width, int height,
  ...
  char **environment;
  check(environment             = (char**)malloc(2*sizeof(char *)));
  int numEnvVars                = 1;
...
//  tt.c_cflag                    = (TTYDEF_CFLAG & ~(CS7|PARENB|HUPCL)) | CS8;
  ...
  //tt.c_oflag                    =  TTYDEF_OFLAG;
  //tt.c_lflag                    =  TTYDEF_LFLAG;
  tt.c_cflag                    = (tt.c_cflag & ~(CS7|PARENB|HUPCL)) | CS8;
  tt.c_cc[VERASE]               = '\x7F';
...
//    check(!pam);
  ...
  if (!*service->cwd || *service->cwd != '/' || chdir(service->cwd)) {
    check(service->cwd          = (const char*)realloc((char *)service->cwd, 2));
...
static void launcherDaemon(int fd) {
  ...
  for (;;) {
    ...
    char *url;
    check(url                 = (char*)calloc(request.urlLength + 1, 1));
  readURL:
